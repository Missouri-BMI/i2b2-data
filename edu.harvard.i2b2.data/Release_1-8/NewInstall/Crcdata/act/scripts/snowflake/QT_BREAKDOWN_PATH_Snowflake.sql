-- Snowflake escaping note:
--   Backslash is the escape character in Snowflake string literals, so every literal
--   backslash must be doubled in this script. Ontology path values therefore use
--   \\\\ for the leading \\TABLE_CD prefix and \\ for each \ path separator,
--   e.g. '\\\\ACT_DEMO\\ACT\\Demographics\\Age\\' stores \\ACT_DEMO\ACT\Demographics\Age\
--   Embedded SQL values are stored with the same text as the PostgreSQL version:
--   the stored literal '\\ACT\\...' resolves to \ACT\... when the CRC executes it
--   (Snowflake LIKE has no default escape character).
...
INSERT INTO qt_breakdown_path (name, value, create_date, update_date, user_id) VALUES ('PATIENT_TOP20MEDS_XML', 'select b.name_char as patient_range, count(distinct a.patient_num) as patient_count
...
INSERT INTO qt_breakdown_path (name, value, create_date, update_date, user_id) VALUES ('PATIENT_TOP20DIAG_XML', 'select b.name_char as patient_range, count(distinct a.patient_num) as patient_count
from {{{DATABASE_NAME}}}observation_fact a, {{{DATABASE_NAME}}}concept_dimension b, {{{DX}}}
    c where a.concept_cd = b.concept_cd
...
INSERT INTO qt_breakdown_path (name, value, create_date, update_date, user_id) VALUES ('PATIENT_TOP20MEDS_CLASS_XML', 'with ingredient_path as (  SELECT  split_part(concept_path, split_part(
...
INSERT INTO qt_breakdown_path (name, value, create_date, update_date, user_id) VALUES ('PATIENT_TOP20DIAG_CLASS_XML', 'with three_dig_path as (
